  ...
  */
  $(document).on("click", ".cart-product-remove .remove-link", function(
    e
  ) {
    e.preventDefault();
    $(this).parents(".cart__list-cntr");
    var item_key = $(this).data("cart-item-key");
    var data = { action: "gsn_remove_item_from_cart", item_key: item_key };
    var response = ajax_call_post(
      data,
      "",
      "",
      function(response) {
        $(".cart__content .cart__list").html(response.item_html);
        $(".cart__content .total__cost").html(response.cart_total);
        $(".cart__icon .cart-indicator").html(response.qty);
      },
      function() {}
    );
  });
  ...
  */
  $(document).click(function(event) {
    if (!$(event.target).closest(".item__cart  .cart.cart-cntr").length) {
      if ($(".item__cart .cart__content").is(":visible")) {
        $(".item__cart .cart__content").slideUp();
        $(".theme-toggle__content--cart-overlay").removeClass("nav-active");
      }
    }

    if (
      !$(event.target).closest(".item__search  .search.search-cntr").length
    ) {
      if ($(".item__search .search__content").is(":visible")) {
        $(".item__search .search__content").slideUp();
    ...
    if ($(".gsn-header").length > 0) {
      ...
      var navMaxHeight = function() {
        if ($(window).innerWidth() <= 800) {
          themeMainNav.css({
            "max-height":
              $(window).innerHeight() - themeHeaderBottom.innerHeight()
          });
        } else {
          themeMainNav.css({
            "max-height": "inherit"
